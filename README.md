...
- **Colours** — `colorPreset`, `waveformColor`, `progressColor` (strings, or `string[]` for gradients). The DOM chrome (button, title, meta text) is themed via CSS variables — `--wfp-button-color`, `--wfp-text-color`, `--wfp-text-secondary-color` — not JS options; override them in your own CSS.
